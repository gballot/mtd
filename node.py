...
class Node:
    ...
    def get_children(self, include_defense=False):
        if include_defense and defense:
            return self.attack_childern + self.goal_children + [self.defense]
        else:
...
class Goal(Node):
    def __init__(
        self, children=None, defense_child=None, attack_childern=None, goal_children=None, operation_type=None, reset=False, name=None, parent=None
    ):
        if children:
...
class Attack(Node):
    def __init__(self, completion_time=None, success_probability=None, activation_cost=None, name=None, parent=None):
        super().__init__(
            parent=parent,
...
class Defense(Node):
    def __init__(self, period=None, success_probability=None, cost=None, name=None, parent=None):
        super().__init__(
            parent=parent,
...
if __name__ == "__main__":
    tree = Goal(
            children=[
                Goal(children=[
                    Attack(completion_time=10, success_probability=0.5, activation_cost=5, name="a0"),
                    Attack(completion_time=3, success_probability=0.2, activation_cost=1),
                    ],
                    operation_type=OperationType.OR,
                    reset=False,
                    name="g1"),
                Attack(completion_time=7, success_probability=0.7, activation_cost=10, name="a2"),
    ...
    tree.set_parents()
    print(f"parent of {tree}'s child {tree.get_children()[1]} is {tree.get_children()[1].parent}")
